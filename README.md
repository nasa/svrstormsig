...
VERSION 4.0.0 of software was released on 17 April 2026 Many user requested updates have been completed. The user can now run OT and AACP model simultaneously in both real-time and archived modes! When run in this mode, the default optimal thresholds and models will be used to detect OTs and AACPs. See Section \ref{software_updates} to see all updates but AACP post-processing saw a significant overhaul! We STRONGLY recommend using AACP object IDs when making identifications instead of the raw AACP probabilities output by the model. This is due to all of the post-processing clean up implementations in this new release. We significantly cut false alarms, particularly some of the edge artifacts we were seeing. In this ReadMe document, we also are now identifying places that users could change to speed up run times (see Section \ref{speed_ups}). We did not apply the run time enhancements in the software release because they are dependent on a users computer specs. The speed ups make use of multi-processing in parallel threads so people should consider the performance of the server or computer that the software is being run on before applying them. 
