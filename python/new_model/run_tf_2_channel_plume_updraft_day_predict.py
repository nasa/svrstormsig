#+
# Name:
#     run_tf_2_channel_plume_updraft_day_predict.py
# Purpose:
...
# Calling sequence:
#     import run_tf_2_channel_plume_updraft_day_predict
#     run_tf_2_channel_plume_updraft_day_predict.run_tf_2_channel_plume_updraft_day_predict()
# Input:
#     None.
# Functions:
#     run_tf_2_channel_plume_updraft_day_predict : Main function to get tensor flow of IR/VIS channels for plumes and updrafts
#     tf_2_channel_plume_updraft_day_predict     : Function uses old model run weights to predict updrafts.
# Output:
...
#     use_glm        : IF keyword set (True), pre-process and run model for IR and GLM data rather than IR and VIS data.
...
#       og_dir = sorted(list(set(og_dir)))                                                                                                                                    #Extract only the unique output IR/VIS/GLM directories given the date
  ...
  for o in og_dir:
    ...
    if use_chkpnt == None:
      ...
      if use_glm == True:                        
        if len(re.split('vis_glm', use_chkpnt)) > 1:                      
          mod0      = 'VIS+GLM'
          mod1      = 'vis_glm'
          use_night = False
    ...
    if len(img_names) > 0:
      ...
      outdir, pthresh0, df = tf_2_channel_plume_updraft_day_predict(dims          = dims,
                                                                    use_glm       = use_glm,
...
def tf_2_channel_plume_updraft_day_predict(dims           = [1, 2000, 2000], 
                                           use_glm        = False,
                                           use_updraft    = True, 
                                           d_str          = None,
                                           date_range     = [], 
...
#      model = unet(input_size = (dims[1],dims[2],1))                                                                                                                        #Run unet function
      ...
      if dims[1] == 500 and dims[2] == 500:
        model = MultiResUnet_new_UPsampling(512,512,2, adam_optimizer = 1e-4)                                                                                               #Run MultiResUnet function
      else:
        model = MultiResUnet_new_UPsampling(512,512,2)                                                                                                                      #Run MultiResUnet function
#        model = MultiResUnet_new_UPsampling(dims[1],dims[2],2, adam_optimizer = 1e-4)                                                                                       #Run MultiResUnet function
    else:  
    ...
    if dims[1] <= 3500 and dims[2] <= 3500:
      if mod_type.lower() == 'unet':  
        model = unet(input_size = (dims[1],dims[2],2))                                                                                                                      #Run unet function
      elif mod_type.lower() == 'multiresunet':  
        model = MultiResUnet(dims[1],dims[2],2)                                                                                                                             #Run MultiResUnet unet function
      else:  
        print('Not set up to do model specified.')  
        print(mod_type)  
        exit()  
...
#          imgs = build_subset_tensor(imgs, 512, 512)                                                                                                                        #Create subset tensor so it does not overwhelm GPU memory
          ...
          if model == None:
            if mod_type.lower() == 'unet':  
              model = unet(input_size = (imgs.shape[1],imgs.shape[2],2))                                                                                                    #Run unet function
            elif mod_type.lower() == 'multiresunet':  
              model = MultiResUnet(imgs.shape[1],imgs.shape[2],2)                                                                                                           #Run MultiResUnet unet function
            else:  
              print('Not set up to do model specified.')  
              print(mod_type)  
              exit()  
...
def main():
  run_tf_2_channel_plume_updraft_day_predict()
