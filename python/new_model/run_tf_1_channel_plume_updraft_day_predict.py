#+
# Name:
#     run_tf_1_channel_plume_updraft_day_predict.py
# Purpose:
...
# Calling sequence:
#     import run_tf_1_channel_plume_updraft_day_predict
#     run_tf_1_channel_plume_updraft_day_predict.run_tf_2_channel_plume_updraft_day_predict()
# Input:
#     None.
# Functions:
#     run_tf_1_channel_plume_updraft_day_predict : Main function to get tensor flow of IR/VIS channels for plumes and updrafts
#     tf_1_channel_plume_updraft_day_predict     : Function uses old model run weights to predict updrafts.
#     append_combined_ncdf_with_model_results    : Function appends the combined netCDF file that is created in pre-processing with the model results.
# Output:
...
def run_tf_1_channel_plume_updraft_day_predict(date1          = None, date2 = None, 
  ...
  if sector[0:4].lower() == 'meso' or sector[0].lower() == 'm': 
    ...
    sector00 = sector0
    if rt == True:
      if date.minute < 10 and sector.lower() == 'full':
...
#       for d in date_list:
#         og_dir.append(os.path.join(outroot, d.strftime("%Y%m%d")))                                                                                                          #Store all output storage directories of the downloaded data
#       og_dir = sorted(list(set(og_dir)))                                                                                                                                    #Extract only the unique output IR/VIS/GLM directories given the date
  ...
  img_path = [datetime.strptime(os.path.basename(o), "%Y%m%d").strftime("%Y%j") for o in og_dir]
  if len(img_path) > 1 and img_path[0] != img_path[-1]:
    img_path = datetime.strptime(os.path.basename(og_dir[0]), "%Y%m%d").strftime("%Y%j") + '-' + datetime.strptime(os.path.basename(og_dir[-1]), "%Y%m%d").strftime("%Y%j")
  else:
    img_path = datetime.strptime(os.path.basename(og_dir[0]), "%Y%m%d").strftime("%Y%j")
  ...
  res_dirs = []
  for o in og_dir:
    d_str2 = datetime.strptime(os.path.basename(o), "%Y%m%d").strftime("%Y%j")                                                                                              #Extract date string in terms of Year and Julian date
    outdate.append(d_str2)
    #Create VIS/IR/GLM combined netCDF file (glm_gridder)
    if verbose == True: print('Combining model input data into a netCDF file for date ' + os.path.basename(o))
    if use_chkpnt == None:
      mod0      = 'IR'
      mod1      = 'ir'
      use_night = True
    ...
    if len(img_names) > 0:
      ...
      if verbose == True: 
        print('Data combined into a netCDF file for date ' + os.path.basename(o))
      ...
      if verbose == True: 
        print('Data numpy files created for date ' + os.path.basename(o))
      ...
      outdir, pthresh0, df = tf_1_channel_plume_updraft_day_predict(dims          = dims,
                                                                    use_glm       = use_glm,
...
def tf_1_channel_plume_updraft_day_predict(dims           = [1, 2000, 2000], 
                                           use_glm        = False,
                                           use_updraft    = True, 
                                           d_str          = None,
                                           date_range     = [], 
...
#       pthresh = df_iou['threshold'][0]                                                                                                                                      #Use threshold that maximizes IoU if threshold not given
  ...
  #  if dims[1] <= 3500 and dims[2] <= 3500:
    if mod_type.lower() == 'unet':  
      ...
      exit()
#      model = unet(input_size = (dims[1],dims[2],1))                                                                                                                        #Run unet function
    elif mod_type.lower() == 'multiresunet':  
      if dims[1] == 500 and dims[2] == 500:
        model = MultiResUnet_new_UPsampling(512,512,1, adam_optimizer = 1e-4)                                                                                               #Run MultiResUnet function
      else:
        model = MultiResUnet_new_UPsampling(512,512,1)                                                                                                                      #Run MultiResUnet function
#        model = MultiResUnet_new_UPsampling(dims[1],dims[2],1, adam_optimizer = 1e-4)                                                                                       #Run MultiResUnet function
    else:  
    ...
    if dims[1] <= 3500 and dims[2] <= 3500:
      if mod_type.lower() == 'unet':  
        ...
        model = MultiResUnet(dims[1],dims[2],1)                                                                                                                             #Run MultiResUnet unet function
      else:  
        print('Not set up to do model specified.')  
        print(mod_type)  
        exit()  
...
#    print(df['date_time'][d])
    ...
    if exist == 0:    
      if use_glm == True:
        imgs = build_imgs(df.loc[d].to_frame().T, [('glm_index',os.path.join(dir_path, 'glm', d_str0 + '_glm.npy'))], dims = [dims[1], dims[2]], bucket_name = p_bucket_name2)                                        
      else:
        if mod1 == 'vis':     
...
#          imgs = build_subset_tensor(imgs, 512, 512)                                                                                                                        #Create subset tensor so it does not overwhelm GPU memory
          ...
          if model == None:
            if mod_type.lower() == 'unet':  
              model = unet(input_size = (imgs.shape[1],imgs.shape[2],1))                                                                                                    #Run unet function
            elif mod_type.lower() == 'multiresunet':  
              model = MultiResUnet(imgs.shape[1],imgs.shape[2],1)                                                                                                           #Run MultiResUnet unet function
            else:  
              print('Not set up to do model specified.')  
              print(mod_type)  
              exit()  
...
#         print('Results shape = ', results.shape)
      if np.sum(na) > 0:
#         if sector.lower() == 'm1' or sector.lower() == 'm2':                                                                                                                #Only occurs if bad mesoscale domain file
#           bad = list(np.argwhere(na))                                                                                                                                       #Find indices of bad scan pixels
#           print('doing it')
#           print(results.shape)
...
#             else:
#               minj = bad[l][2]-15
#             if bad[l][2] >= dims[2]-15:
#               maxj = dims[2]
#             else:
#               maxj = bad[l][2]+16
#             results[0, mini:maxi, minj:maxj, 0] = 0
