#+
# Name:
#     mtg_combine_ir_glm_vis_parallel.py
# Purpose:
#     This is a script to create the combined netCDF file for MTG data files
# Calling sequence:
#     import mtg_combine_ir_glm_vis_parallel
# Input:
#     None.
# Functions:
#     extract_universal_vars          : Extracts smoothed/resized GLM and resized IR data on VIS data grid
#     mtg_combine_ir_glm_vis_parallel : Creates and write netCDF file that combines IR, VIS, and GLM data
# Output:
...
#     proj_cache           : Dictionary to cache projection file data so it is only read from disk once 
#                            DEFAULT = None
...
#                              2026-03-27. (Added projection file caching via proj_cache keyword)
#-
...
_MTG_PROJECTION_CACHE = {}

def _get_mtg_projection_data(proj_file, xy_bounds, sat, no_write_vis, scan_mode, proj_cache, verbose):
    """
    Load MTG projection file data and cache the result so the 850 MB file is
    only read from disk once per unique (proj_file, xy_bounds, sat,
    no_write_vis, scan_mode) combination per process.

    All MTG domain sectors are fixed grid positions so no sector center
    coordinates are needed in the cache key unlike GOES mesoscale.

    When xy_bounds are specified the returned lat_arr and lon_arr are already
    pre-subsetted to the domain so no further lat/lon slicing is needed by
    the caller. x_inds and y_inds are still returned so the caller can subset
    the per-file data arrays (bt, vis_img etc.) which are always full size.

    Args:
        proj_file    : Full path to the MTG satellite projection netCDF file.
        xy_bounds    : List/array of [x0, y0, x1, y1] domain bounds, or []
                       for the full domain.
        sat          : Satellite name string (e.g. 'mtg1').
        no_write_vis : Boolean - if True, use native IR grid instead of VIS.
        scan_mode    : String scan mode (always 'full' for MTG).
        proj_cache   : Dict-like object to use as the cache. Should be the
                       module-level _MTG_PROJECTION_CACHE for per-process
                       caching. Do NOT use a multiprocessing.Manager().dict()
                       as the pipe serialization overhead for large numpy
                       arrays is far more expensive than each worker reading
                       the 850 MB file once.
    ...
    """
    # Build cache key - no center coords needed since all MTG sectors are fixed
    ...
    if verbose:
        print('Reading MTG projection file from disk and caching result: ' + proj_file)
    ...
    # When xy_bounds are specified, pre-subset lat/lon arrays
    if len(xy_bounds) > 0:
        ...
        lat_sub = np.array(lat_arr[x0:x1, y0:y1])                                                                                          #Pre-subsetted lat array - static across all files for fixed MTG sectors
        lon_sub = np.array(lon_arr[x0:x1, y0:y1])                                                                                          #Pre-subsetted lon array - static across all files for fixed MTG sectors
    ...
    # Store in cache for all future calls in this process
    proj_cache[cache_key] = result
    return(result)


def mtg_combine_ir_glm_vis_parallel(infile               = os.path.join('..', '..', '..', 'mtg-data', '20241029', 'W_XX-EUMETSAT-Darmstadt,IMG+SAT,MTI1+FCI-1C-RRAD-HRFI-FD--CHK-TRAIL---NC4E_C_EUMT_20241029140421_IDPFI_OPE_20241029140007_20241029140017_N__C_0085_0001.nc'),
    ...
    # If no external cache dict was supplied, fall back to the module-level per-process cache
    if proj_cache is None:
        proj_cache = _MTG_PROJECTION_CACHE
    ...
    out_nc = os.path.join(layered_dir, f'OR_{ins}_{proc_lvl}_{domain_sector}_COMBINED_s{ds0.strftime("%Y%j%H%M%S0")}_e{de0.strftime("%Y%j%H%M%S0")}_c{dc0.strftime("%Y%j%H%M%S0")}.nc')
...
#         t0 = time.time()
        ...
        # Capture is_append before any file writing
        is_append = False                                                                                                                   #Initialize before try/except ***
        if os.path.isfile(out_nc) and append_nc and not rewrite_nc:
            try:                                                                                                                            #Ensure combined netCDF file is not corrupt before trying to append to it
                f         = netCDF4.Dataset(out_nc,'a', format='NETCDF4')        
                is_append = True                                                                                                            #Set True if file opened successfully
            except:
                print('File existed but was corrupted so am rewriting it.')
                is_append = False                                                                                                           #Corrupted file falls through to rewrite netCDF
        ...
        if is_append:
            ...
            if ('solar_zenith_angle' not in keys0) and (not no_write_vis or not no_write_cirrus or not no_write_snowice):
                ...
                proj_file_cache = os.path.join(proj_dir, sat + '_satellite' + pat_proj + 'full_scan_projections.nc')
                scan_mode_cache = 'full'
                cache_key       = (proj_file_cache, tuple(xy_bounds), sat, no_write_vis, scan_mode_cache)                                   # *** NEW ***
                if cache_key in proj_cache:
                    _, _, _, _, lat_arr, lon_arr = proj_cache[cache_key]                                                                    #Pull pre-subsetted lat/lon from cache
                    got_latlon_from_cache = True
                    if verbose:
                        print('Using cached MTG lat/lon arrays for solar zenith angle calculation (append mode)')
                if not got_latlon_from_cache:
                    # Fallback - same as original code if cache miss
                    lon_arr = np.asarray(f.variables['longitude'])                                                                          #Read longitude values from file if cache miss
                    lat_arr = np.asarray(f.variables['latitude'])                                                                           #Read latitude values from file if cache miss
...
#             t0 = time.time()
            ...
            #Replace direct Dataset open + get_lat_lon_subset_inds calls with caching wrapper
            if len(xy_bounds) > 0:
                if not os.path.exists(proj_file):
                    print('Satellite projection file for the specified indices not found????')
                    print(proj_file)
                    exit()
                x_inds, y_inds, proj_extent_new, og_shape, lat_arr, lon_arr = _get_mtg_projection_data(proj_file, xy_bounds, sat, no_write_vis, scan_mode, proj_cache, verbose)
                # Note: lat_arr/lon_arr are now pre-subsetted. og_shape check skipped since lat/lon are pre-subsetted.
            else:
                x_inds, y_inds, proj_extent, og_shape, lat_arr, lon_arr = _get_mtg_projection_data(proj_file, [], sat, no_write_vis, scan_mode, proj_cache, verbose)
...
#             t0 = time.time()
            ...
            with warnings.catch_warnings():
              ...
              zen   = np.radians(90.0)-get_position(date, lon_arr, lat_arr)['altitude']
              coszen = np.cos(zen)
#             print('Time to calculate solar zenith angle = ' + str(time.time() - t0) + 'sec')
        ...
        if len(keys0) <= 0:
            ...
            f.x_inds              = x_inds                                                                                                  #Write x indices as global variable
            f.y_inds              = y_inds                                                                                                  #Write y indices as global variable
        ...
        if not universal_file:
            ...
            with warnings.catch_warnings():
              ...
              # Data arrays (bt, vis_img etc.) always come from raw MTG files at full resolution and always need subsetting when xy_bounds
              if len(xy_bounds) > 0:
        ...
        if 'ir_brightness_temperature' not in keys1:
            bt[rm_inds]               = np.nan                                                                                              #Remove likely untrue values by setting them to nan
...
#         print('Time to scale and offset the data = '+ str(time.time() -t0) + 'sec')
        ...
        shape_arr = lon_arr.shape[0]                                                                                                        #Use lon_arr computed during write branch
        ...
        zen    = None
        coszen = None
        print('Time to write data combined netCDF file = ' + str(time.time() - t00) + 'sec')
    else:
        with Dataset(out_nc, 'r') as combined_data:
            #Read only the dimension size rather than loading the entire longitude array
...
def get_lat_lon_from_mtg(dataset, x, y, extract_proj_coordinates = False, verbose = True):
    ...
    r_eq_sq     = r_eq ** 2
    r_pol_sq    = r_pol ** 2
    sin_lat_sq  = np.square(sin_lat_rad)
    cos_lat_sq  = np.square(cos_lat_rad)
    a_var       = (np.square(sin_lon_rad) + np.square(cos_lon_rad) * (cos_lat_sq + (r_eq_sq / r_pol_sq) * sin_lat_sq))
